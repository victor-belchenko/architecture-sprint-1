...
# Комментарии на замечания ревьюера

## Список микрофронотов
1. Хост компонент находится в папке frontend
2. Микрофронт ответственный за работу с карточками, который должен далее развиваться так чтобы за карточкой разворачивалась разнообразная функциональность для описания путешествий к этому месту, находится в папке remote_front 
## Запуск приложения

1. Сначала запускается удаленное приложение из папки remote_front командой npm run start на порту 3002
2. Затем запускается хост из папки frontend командой npm run start на порту 3000
3. Изменения видны при создании новой карточки и просмотре существующей карточки


## В папке microfrontend пусто

В задании не было однозначного требования размещать новые компоненты в папке microfrontend, поэтому я создал новую папку remote_front 
